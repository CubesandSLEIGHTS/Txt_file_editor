from os import chdir, name, system, path, makedirs
...
def create_txt():
    fn = input('What do you want your file to be called? (Use underscores [ _ ] instead of spaces) ')
    fn += '.txt'
    a = open(fn, 'w')
    a.close()
    write_txt(fn)
...
def add_to_txt(fn):
    lst = []
    txt = ''
    print('What do you want to add to the .txt file?')
    while True:
        if txt.upper() == 'END':break
        else:
            lst.append('\n' + txt)
